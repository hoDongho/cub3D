...
/*   Updated: 2022/10/13 20:08:11 by littley          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	draw_sprite(t_info *info, t_sprite_tool *tool)
{
	...
	{
		tool->tex_x = (256 * (i - (-tool->s_width / 2 + tool->s_screen_x)) \
						* P_WIDTH / tool->s_width) / 256;
			...
			&& tool->t_y < info->z_buffer[i])
		{
			...
			{
				d = (j - tool->v_move_screen) * 256 - W_HEIGHT * 128 \
					+ tool->s_height * 128;
	...
	t_sprite_tool *tool, t_sprite_data *data)
{
	...
	tool->s_height = abs((int)(W_HEIGHT / tool->t_y)) / VDIV;
	...
	tool->s_width = abs((int)(W_HEIGHT / tool->t_y)) / UDIV;
